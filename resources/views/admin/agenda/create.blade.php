...
<div class="p-6 max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-800">Tambah Agenda</h1>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('admin.agenda.store') }}" method="POST" class="space-y-5">
            @csrf

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">Judul Kegiatan</label>
                <input type="text" name="judul" value="{{ old('judul') }}" placeholder="Contoh: Jatizi Goes To Camp"
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" placeholder="Detail singkat kegiatan (opsional)"
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-slate-100">
                ...
                <a href="{{ route('admin.agenda.index') }}" class="px-5 py-2.5 bg-slate-100 text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-200 transition">
                    Batal
                </a>
            </div>
        </form>
