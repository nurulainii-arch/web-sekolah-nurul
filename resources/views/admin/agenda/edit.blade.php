...
<div class="p-6 max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('admin.agenda.update', $agenda->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3"
                          class="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ old('deskripsi', $agenda->deskripsi) }}</textarea>
            </div>
