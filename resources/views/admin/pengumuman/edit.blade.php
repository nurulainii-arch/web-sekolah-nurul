...
<div class="p-6 max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('admin.pengumuman.update', $pengumuman->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="kategori" class="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                    <option value="Mendesak" {{ old('kategori', $pengumuman->kategori) == 'Mendesak' ? 'selected' : '' }}>Mendesak</option>
                    <option value="Siaga Bencana" {{ old('kategori', $pengumuman->kategori) == 'Siaga Bencana' ? 'selected' : '' }}>Siaga Bencana</option>
            ...
            <div>
                ...
                <textarea name="isi" rows="4"
                          class="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ old('isi', $pengumuman->isi) }}</textarea>
            </div>
